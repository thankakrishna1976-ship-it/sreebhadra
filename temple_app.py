...
def generate_pdf(receipt_no, devotee_name, devotee_address, service, amount, trans_date):
    ...
    y_start = 780 
    c.setFont("Helvetica-Bold", 16); c.drawCentredString(300, y_start, TEMPLE_NAME_FULL)
    c.setFont("Helvetica", 10); c.drawCentredString(300, y_start - 15, TRUST_DETAILS)
    c.line(50, y_start - 60, 550, y_start - 60)
    c.setFont("Helvetica-Bold", 12); c.drawString(50, y_start - 90, f"RECEIPT No: #{receipt_no}"); c.drawString(400, y_start - 90, f"DATE: {trans_date}")
    c.setFont("Helvetica", 12); c.drawString(50, y_start - 130, f"Devotee: {devotee_name}")
    c.drawString(50, y_start - 180, f"Service: {service}"); c.setFont("Helvetica-Bold", 14); c.drawString(50, y_start - 210, f"AMOUNT: Rs. {float(amount):,.2f}/-")
...
def render_navigation_bar():
    ...
    nav_items = ALL_PAGES if st.session_state.role == ADMIN_ROLE else {k: v for k, v in ALL_PAGES.items() if k in st.session_state.get('rights', ["Home Dashboard"])}
...
def render_news_ticker():
    ...
    st.markdown(f"""<style>.ticker-wrap {{ background: #800000; padding: 10px; border: 2px solid #FFD700; overflow: hidden; }} .ticker {{ white-space: nowrap; animation: marquee 30s linear infinite; color: #FFD700; font-weight: bold; display: inline-block; padding-left: 100%; }} @keyframes marquee {{ 0% {{ transform: translateX(0); }} 100% {{ transform: translateX(-200%); }} }}</style><div class="ticker-wrap"><div class="ticker">{text}</div></div><br>""", unsafe_allow_html=True)
...
if st.session_state.current_page == "Home Dashboard":
    ...
    # --- FIXED: SAFE DATE CONVERSION (The Error Fix) ---
    if not df_t.empty:
        df_t['date_obj'] = pd.to_datetime(df_t['date'], errors='coerce').dt.date
    if not df_e.empty:
        df_e['date_obj'] = pd.to_datetime(df_e['payment_date'], errors='coerce').dt.date

    def get_stats(sd, ed):
        inc = df_t[(df_t['date_obj'] >= sd) & (df_t['date_obj'] <= ed)]['amount'].sum() if not df_t.empty else 0
        exp = df_e[(df_e['date_obj'] >= sd) & (df_e['date_obj'] <= ed)]['amount'].sum() if not df_e.empty else 0
        return inc, exp

    stats_list = [("Daily", today, today), ("Weekly", start_week, today), ("Monthly", start_month, today), ("Yearly", start_year, today)]
    for lbl, sd, ed in stats_list:
        i, e = get_stats(sd, ed)
        st.subheader(f"📊 {lbl} Overview")
        col1, col2, col3 = st.columns(3)
        col1.metric("Income", f"₹ {i:,.2f}"); col2.metric("Expense", f"₹ {e:,.2f}"); col3.metric("Net", f"₹ {i-e:,.2f}")
        st.divider()
    st.metric("Total Devotee Families", len(df_f))

elif st.session_state.current_page == "Enroll":
    st.header("Devotee Enrollment")
    t1, t2 = st.tabs(["📝 Manual", "📥 Bulk Upload"])
    with t1:
    ...
    with t2:
        ...
        sample = pd.DataFrame({"Name": ["Example"], "Phone": ["9000000000"], "Address": ["Address Here"], "Star": ["Ashwini"], "DOB": ["1990-01-01"]})
        st.download_button("📥 Template", to_excel(sample), "enrollment_template.xlsx")
    ...
    if not f_data.empty:
        opts = {}
        for _, f in f_data.iterrows(): opts[f"{f['head_name']} (Head) | 📱 {f['phone']}"] = {"id": f['id'], "name": f['head_name'], "addr": f['address']}
        for _, m in m_data.iterrows():
            h = f_data[f_data['id'] == m['family_id']]
            hp = h['phone'].values[0] if not h.empty else ""
            opts[f"{m['member_name']} | 📱 {m['phone'] if m['phone'] else hp}"] = {"id": m['family_id'], "name": m['member_name'], "addr": h['address'].values[0] if not h.empty else ""}
        ...
        if sel:
            ...
            if st.button("Generate Bill"):
                ...
                if res: 
                    st.success("Generated!"); pdf = generate_pdf(res.data[0]['id'], d['name'], d['addr'], s_sel, srv['price'], str(date.today()))
                    st.download_button("📥 PDF Receipt", pdf, f"Receipt_{res.data[0]['id']}.pdf")

elif st.session_state.current_page == "Samayavakuppu":
    st.header("Samayavakuppu Student Bond Management")
    ta, tb = st.tabs(["📝 Student Bond Entry", "📋 Records"])
    with ta:
        with st.form("bond_f", clear_on_submit=True):
            ...
            if st.form_submit_button("Save Student Bond"):
                if sn and bn:
    ...
    with tb:
        st.dataframe(get_data("student_bonds"), use_container_width=True)
    ...
    ledger = []
    if not df_t.empty:
        for _, r in df_t.iterrows(): ledger.append({"Date": r['date'], "Type": "Income", "Amount": r['amount']})
    if not df_e.empty:
        for _, r in df_e.iterrows(): ledger.append({"Date": r['payment_date'], "Type": r['expense_type'], "Amount": r['amount']})
    if ledger:
        df_l = pd.DataFrame(ledger)
        st.subheader("📊 Category-Wise Summary")
        st.table(df_l.groupby('Type')['Amount'].sum())
        st.bar_chart(df_l.groupby('Type')['Amount'].sum())
        st.dataframe(df_l, use_container_width=True)

elif st.session_state.current_page == "Search":
    st.header("Search Devotees")
    df_s = get_data("families"); q = st.text_input("Search Name or Phone")
    if not df_s.empty:
    ...
    if st.session_state.role == ADMIN_ROLE:
        ...
        if st.button("Create"): run_supabase_insert("users", {"username": un, "password_hash": hash_password(up), "role": "user"}); st.rerun()
        st.dataframe(get_data("users", "username, role"))
