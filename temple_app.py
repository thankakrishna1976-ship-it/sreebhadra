...
def generate_financial_pdf(income_df, expense_df, title, t_inc, t_exp, t_net):
    ...
    st_table = Table(summary_data, colWidths=[150, 150])
    st_table.setStyle(TableStyle([('BACKGROUND', (0, 0), (0, -1), colors.lightgrey), ('GRID', (0,0), (-1,-1), 0.5, colors.black), ('FONTNAME', (0,0), (-1,-1), 'Helvetica-Bold')]))
    story.append(st_table); story.append(Spacer(1, 24)); doc.build(story); buffer.seek(0)
    return buffer
...
def verify_user(username, password):
    ...
    if res.data:
        ...
        if user['password_hash'] == hash_password(password):
            return True, user['role'], user.get('rights', 'Home Dashboard').split(',')
    return False, None, None

def generate_pdf(receipt_no, devotee_name, devotee_address, service, amount, trans_date, manual_no, book_no):
    buffer = io.BytesIO(); c = canvas.Canvas(buffer, pagesize=letter); y = 780 
    try:
        if os.path.exists(LOGO_PATH): c.drawImage(ImageReader(LOGO_PATH), 50, y - 50, width=50, height=50)
    except: pass
    ...
    c.line(50, y - 60, 550, y - 60); dy = y - 90
...
if st.session_state.current_page == "Home Dashboard":
    ...
    with c1:
        ...
        if not servs.empty:
            ...
            st.markdown(f"**Current Bill Value: ₹ {float(srv['price']):,.2f}**")
    ...
    # --- ADDED: SERVICE-WISE REPORT TOGGLE ---
    report_type = st.radio("Choose Report View:", ["Transaction Ledger", "Service-wise Income Report"], horizontal=True)
    ...
    if not df_trans.empty:
        ...
        # Merge to get Service Names
        if not df_serv.empty and not df_trans.empty:
            df_trans = df_trans.merge(df_serv[['id', 'service_name']], left_on='service_id', right_on='id', how='left', suffixes=('', '_svc'))

    # Display Report based on Selection
    if report_type == "Transaction Ledger":
        st.subheader("All Transactions (Period Selection)")
        st.dataframe(df_trans if not df_trans.empty else "No data.")
    else:
        st.subheader("Income Breakdown by Service Name")
        if not df_trans.empty and 'service_name' in df_trans.columns:
            ...
            summary.columns = ['Service Name', 'Receipt Count', 'Total Collection (₹)']
            st.table(summary)
            st.metric("Total Income (Period)", f"₹ {summary['Total Collection (₹)'].sum():,.2f}")
        else:
            st.info("No service income records found for this period.")

elif st.session_state.current_page == "Samayavakuppu":
    page_header(); render_navigation_bar(); st.header("Samayavakuppu Student Bonds")
        ...
        with st.form("sb_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Save Student Record"):
                if sn and bn:
                    ...
                    if run_supabase_insert("student_bonds", data): st.success("Saved Successfully!"); st.rerun()
    with t2:
